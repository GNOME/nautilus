...
test_both_null (void)
{
    g_assert_true (nautilus_file_selection_equal (NULL, NULL) == TRUE);
}
...
test_either_null (void)
{
    ...
    g_list_free_full (selection, (GDestroyNotify) g_object_unref);
...
test_one_file_different (void)
{
    ...
    g_list_free_full (first_selection, (GDestroyNotify) g_object_unref);
    g_list_free_full (second_selection, (GDestroyNotify) g_object_unref);
    g_object_unref (directory);
}

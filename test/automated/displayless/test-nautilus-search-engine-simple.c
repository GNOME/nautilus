             ...
             gpointer                      user_data)
{
    ...
    file = g_file_get_child (location, "engine_simple");
    g_file_delete (file, NULL, NULL);

    file = g_file_get_child (location, "engine_simple_directory");
    file = g_file_get_child (file, "child");
    g_file_delete (file, NULL, NULL);
    file = g_file_get_child (location, "engine_simple_directory");
    g_file_delete (file, NULL, NULL);

    file = g_file_get_child (location, "engine_simple_directory");
    file = g_file_get_child (file, "engine_simple_child");
    g_file_delete (file, NULL, NULL);
    file = g_file_get_child (location, "engine_simple_directory");
    g_file_delete (file, NULL, NULL);

    file = g_file_get_child (location, "engine_simple_second_directory");
    file = g_file_get_child (file, "engine_simple_child");
    g_file_delete (file, NULL, NULL);
    file = g_file_get_child (location, "engine_simple_second_directory");
    g_file_delete (file, NULL, NULL);
      ...
      char *argv[])
{
    ...
    file = g_file_get_child (file, "simple_child");
    out = g_file_create (file, G_FILE_CREATE_NONE, NULL, NULL);
    if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_EXISTS))
    {
        g_object_unref (out);
    ...
    file = g_file_get_child (location, "simple_directory");
    g_file_make_directory (file, NULL, NULL);

...
{
    g_autoptr (GFile) file = g_file_new_for_uri ("tmp:///simple_file");
...
{
    g_autoptr (GFile) parent = g_file_new_for_uri (ROOT_DIR);
    g_autoptr (GFile) child = g_file_get_child (parent, "empty_dir");
...
{
    g_autoptr (GFile) root = g_file_new_for_uri (ROOT_DIR);
    g_autoptr (GFile) parent_dir = g_file_get_child (root, "parent_dir");
    g_autoptr (GFile) child_file = g_file_get_child (parent_dir, "child_file");

    g_assert_true (parent_dir);
    g_assert_true (child_file);

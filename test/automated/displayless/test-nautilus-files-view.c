                            ...
                            GEqualFunc  equal_func)
{
    ...
    {
        if (!g_ptr_array_find_with_equal_func (b, a->pdata[i], equal_func, NULL))
        {
            return FALSE;
        }
    }
...
static void
wait_for_view_load (NautilusFilesView *files_view)
{
    while (nautilus_files_view_get_loading (files_view))
    {
                          ...
                          GPtrArray         *data_files)
{
    ...
    {
        NautilusFile *file = NAUTILUS_FILE (link->data);
        GFile *removed_location = nautilus_file_get_location (file);

        g_assert_nonnull (removed_location);

        g_ptr_array_add (data_files, removed_location);
    }
}
...
test_remove_files (void)
{
    ...
    g_autoptr (GPtrArray) file_arr = g_ptr_array_new_full (file_count, g_object_unref);
    g_autoptr (GPtrArray) deleted_files_arr = g_ptr_array_new_full (deleted_file_count,
                                                                    g_object_unref);
    g_autoptr (GPtrArray) callback_arr = g_ptr_array_new_full (file_count, g_object_unref);
    ...
    for (guint i = 0; i < file_count; i++)
    {
        ...
        GFile *file = g_file_get_child (tmp_location, file_name);
    ...
    for (guint i = 0; i < deleted_file_count; i++)
    {
        ...
        g_file_delete (file_arr->pdata[i], NULL, &error);

        g_assert_null (error);
        g_ptr_array_add (deleted_files_arr, g_object_ref (file_arr->pdata[i]));
    }

    while (!ptr_arrays_equal_unordered (deleted_files_arr, callback_arr, (GEqualFunc) g_file_equal))
    {
        g_main_context_iteration (NULL, FALSE);
    }
                        ...
                        GPtrArray         *data_files)
{
    ...
    {
        GFile *added_location = nautilus_file_get_location (NAUTILUS_FILE (link->data));

        g_assert_nonnull (added_location);

        g_ptr_array_add (data_files, added_location);
    }
}
...
test_add_files (void)
{
    ...
    for (guint i = 0; i < file_arr->len; i++)
    {
        g_autoptr (GFileOutputStream) out = g_file_create (file_arr->pdata[i],
                                                           G_FILE_CREATE_NONE, NULL, NULL);

        g_assert_nonnull (out);
    }

    while (!ptr_arrays_equal_unordered (file_arr, callback_arr, (GEqualFunc) g_file_equal))
    {
        g_main_context_iteration (NULL, FALSE);
    }
...
test_load_dir (void)
{
    ...
    g_autoptr (GFile) tmp_location = g_file_new_for_path (test_get_tmp_dir ());
    g_autofree gchar *uri = g_file_get_uri (tmp_location);
    gboolean started_loading = FALSE, ended_loading = FALSE;
    const guint file_count = 10;
    ...
    nautilus_view_set_location (NAUTILUS_VIEW (files_view), tmp_location);
    g_signal_connect_swapped (files_view, "begin-loading", G_CALLBACK (set_true), &started_loading);
    g_signal_connect_swapped (files_view, "end-loading", G_CALLBACK (set_true), &ended_loading);

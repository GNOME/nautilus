...
/* HACK: Use internals of NautilusFileChooser directly */
...
static void
emit_clicked_signal (gpointer user_data)
{
    ...
    g_signal_emit_by_name (GTK_WIDGET (self->accept_button), "clicked", NULL);
}
                                    ...
                                    gpointer   user_data)
{
    ...
    gtk_test_widget_wait_for_draw (self->accept_button);

    accept_source_id = g_idle_add_once (emit_clicked_signal, self);
}

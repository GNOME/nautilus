...
all_files_are_ready (GList *files)
{
    for (GList *l = files; l != NULL; l = l->next)
    {
        ...
        if (!nautilus_file_check_if_ready (file, NAUTILUS_ATTRIBUTE_THUMBNAIL_BUFFER))
        {
            return FALSE;
        }
    }
...
test_image (void)
{
    ...
    ITER_CONTEXT_WHILE (!nautilus_file_check_if_ready (image1_file,
    ...
    ITER_CONTEXT_WHILE (!nautilus_file_check_if_ready (image2_file,
                                                       NAUTILUS_ATTRIBUTE_THUMBNAIL_BUFFER));
    gtk_test_widget_wait_for_draw (GTK_WIDGET (properties_window));
...
test_images (void)
{
    ...
    g_autolist (GFile) thumb_image_locations = file_hierarchy_get_files_list (hier, "thumb_", FALSE);
    g_autolist (NautilusFile) image_files = NULL;
    g_autolist (NautilusFile) thumb_image_files = NULL;
    g_autoptr (GtkWindow) properties_window = NULL;
    ...
    image_files = g_list_reverse (image_files);
    ...
    for (GList *l = thumb_image_locations; l != NULL; l = l->next)
    {
        GFile *location = l->data;

        make_image_file_with_mtime (location, 1, TRUE);
        thumb_image_files = g_list_prepend (thumb_image_files, nautilus_file_get (location));
    }
    thumb_image_files = g_list_reverse (thumb_image_files);
    properties_window = nautilus_properties_present_window (thumb_image_files, NULL);

    ITER_CONTEXT_WHILE (!all_files_are_ready (thumb_image_files));
    gtk_test_widget_wait_for_draw (GTK_WIDGET (properties_window));

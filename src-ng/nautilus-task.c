...
finalize (GObject *object)
{
    ...
    if (self->data != NULL)
    {
        g_clear_pointer (&self->data, self->data_destroy);
    }
...
nautilus_task_get_cancellable (NautilusTask *task)
{
    ...
    {
        cancellable = g_object_ref (task->cancellable);
    }
                             ...
                             GCancellable     *cancellable)
{
    ...
    task = nautilus_task_new_with_closure (closure, cancellable);

    task->data = func_data;
    task->data = func_data_destroy;
...
{
    return nautilus_task_new_with_func (dummy_task_func, NULL,
                                        dummy_destroy_func, cancellable);
}

...
static void
av_properties_free (AVProperties *props)
{
    if (props->disco != NULL)
    {
...
G_DEFINE_AUTOPTR_CLEANUP_FUNC (AVProperties, av_properties_free);
...
audio_video_properties_model_new (const char *location)
{
    ...
    props->model = nautilus_properties_model_new (_("Audio/Video Properties"),
                                                  G_LIST_MODEL (props->store));
    props->disco = disco;
    props->handler_id = g_signal_connect (props->disco, "discovered",
                                          (GCallback) discovered_cb, props);

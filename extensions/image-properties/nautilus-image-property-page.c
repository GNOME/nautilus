...
#include "nautilus-image-property-page.h"
...
struct _NautilusImagePropertyPage
{
    GtkGrid parent;
...
G_DEFINE_TYPE (NautilusImagePropertyPage, nautilus_image_property_page, GTK_TYPE_GRID);

static void
finalize (GObject *object)
{
    NautilusImagePropertyPage *page;

    page = NAUTILUS_IMAGE_PROPERTY_PAGE (object);
    ...
    G_OBJECT_CLASS (nautilus_image_property_page_parent_class)->finalize (object);
}

static void
nautilus_image_property_page_class_init (NautilusImagePropertyPageClass *klass)
{
...
static void
append_item (NautilusImagePropertyPage *page,
             const char                *name,
             const char                *value)
{
...
static void
nautilus_image_property_page_init (NautilusImagePropertyPage *page)
{
    ...
    gtk_grid_set_column_spacing (GTK_GRID (page->grid), 18);
    append_item (page, _("Loading…"), NULL);
    gtk_container_add (GTK_CONTAINER (scrolled_window), page->grid);

    gtk_widget_show_all (GTK_WIDGET (page));
}

static void
append_basic_info (NautilusImagePropertyPage *page)
{
...
static void
append_gexiv2_tag (NautilusImagePropertyPage  *page,
...
static void
append_gexiv2_info (NautilusImagePropertyPage *page)
{
    double longitude;
    double latitude;
    double altitude;
...
static void
load_finished (NautilusImagePropertyPage *page)
{
    GtkWidget *label;
...
void
nautilus_image_property_page_load_from_file_info (NautilusImagePropertyPage *page,
                                                  NautilusFileInfo          *info)
{
    g_autofree char *uri = NULL;
    ...
    g_return_if_fail (NAUTILUS_IS_IMAGE_PROPERTY_PAGE (page));
    ...
    else
    {
        ...
        if (path != NULL)
        {
            ...
            if (!gexiv2_metadata_open_path (page->md, path, &error))
            {
...
NautilusImagePropertyPage *
nautilus_image_property_page_new (void)
{
    return g_object_new (NAUTILUS_TYPE_IMAGE_PROPERTY_PAGE,
                         "margin-bottom", 6,
                         "margin-end", 12,
                         "margin-start", 12,
                         "margin-top", 6,

                    ...
                    gpointer      user_data)
{
    ...
    return nautilus_file_compare_for_sort_by_attribute (file_a, file_b,
                                                        "date_accessed",
                                                        FALSE,
                                                        TRUE /* reversed */);
}
             ...
             gpointer                  user_data)
{
    ...
    switch (get_section (item))
    {
        ...
        {
            gtk_label_set_label (GTK_LABEL (label), _("Avaliable on Current Network"));
            /* TODO: Use network name from NMClient:primary-connection:id to
             * match design mockup: "Avaliable on Network1234"
             */
        }
        break;
...
create_view_ui (NautilusNetworkView *self)
{
    ...
    g_object_bind_property (self, "model", list_view, "model", G_BINDING_SYNC_CREATE);

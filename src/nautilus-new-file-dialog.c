                              ...
                              gpointer           callback_data)
{
    ...
    if (!nautilus_application_is_sandboxed ())
    {
        ...
        {
            { self->document_action_row, "application/vnd.oasis.opendocument.text" },
            { self->spreadsheet_action_row, "application/vnd.oasis.opendocument.spreadsheet" },
            { self->presentation_action_row, "application/vnd.oasis.opendocument.presentation" },
        };

        for (guint i = 0; i < G_N_ELEMENTS (documents); i++)
        {
            GtkWidget *action_row = GTK_WIDGET (documents[i].row);
            GList *recommended_apps = g_app_info_get_recommended_for_type (documents[i].mime_type);

...
static GPtrArray *fles_to_prioritize = NULL;

static GPtrArray *
get_priority_array (void)
{
    if (fles_to_prioritize == NULL)
    {
        fles_to_prioritize = g_ptr_array_new_with_free_func (free_weak_ref);
    }

    return fles_to_prioritize;
}

static void
prioritize_idle_callback (gpointer data)
{
    g_autoptr (GPtrArray) priority_files = g_steal_pointer (&fles_to_prioritize);

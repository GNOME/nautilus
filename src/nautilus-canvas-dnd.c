                        ...
                        gpointer          data)
{
    ...
    nautilus_drag_drag_data_get_from_cache (drag_info->selection_cache, context, selection_data);
}
    ...
    if (drop_target_icon != NULL)
    {
        ...
        if (icon_uri != NULL)
        {
            ...
            if (!nautilus_drag_can_accept_data (file,
                                                container->details->dnd_info->drag_info.selection_data,
                                            ...
                                            int                      y)
{
    ...
    if (target == g_intern_static_string (NAUTILUS_ICON_DND_GNOME_ICON_LIST_TYPE))
    {
        if (container->details->dnd_info->drag_info.selection_list != NULL)
        {
            return nautilus_get_drop_actions_for_icons (drop, drop_target,
                                                        container->details->dnd_info->drag_info.selection_list,
                                                        0);
        }
    }
    ...
    {
        return nautilus_get_drop_actions_for_uri (drop, drop_target);
    }
                      ...
                      int        y)
{
    ...
    nautilus_canvas_container_position_shadow (container, x, y);
    nautilus_canvas_dnd_update_drop_target (container, x, y);
    set_up_auto_scroll_if_needed (container);
    /* Find out what the drop actions are based on our drag selection and
     * the drop target.
     */
    actions = nautilus_canvas_container_get_drop_actions (container, drop, x, y);
    if (actions != 0)
    {
        g_autofree char *uri = NULL;
...
nautilus_canvas_dnd_init (NautilusCanvasContainer *container)
{
    ...
    dnd_info->drag_info.targets = gdk_content_formats_new (drag_types, G_N_ELEMENTS (drag_types));
    dnd_info->drag_info.targets = gtk_content_formats_add_uri_targets (dnd_info->drag_info.targets);

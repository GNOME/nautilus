                 ...
                 GError                          *error)
{
    ...
    if (error && !g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
    {
        ...
        nautilus_search_provider_finished (NAUTILUS_SEARCH_PROVIDER (self), TRUE, self->run_id);
    }
    else
    {
        nautilus_search_provider_finished (NAUTILUS_SEARCH_PROVIDER (self), FALSE, self->run_id);

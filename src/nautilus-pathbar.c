...
#define NAUTILUS_PATH_BAR_LABEL_MAX_WIDTH_CHARS 48
...
nautilus_path_bar_update_button_appearance (ButtonData *button_data)
{
    ...
    if (button_data->label != NULL && icon == NULL)
    {
        gtk_style_context_add_class (gtk_widget_get_style_context (button_data->button),
                                     "text-button");
    }
...
prepare_button_label (GtkWidget *label)
{
    ...
    gtk_label_set_max_width_chars (GTK_LABEL (label), NAUTILUS_PATH_BAR_LABEL_MAX_WIDTH_CHARS);
}

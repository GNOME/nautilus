                          ...
                          NautilusSearchPopover *self)
{
    ...
    {
        gtk_stack_set_visible_child_name (self->type_dialog_stack, "new_page");
    ...
    result_count = g_list_model_get_n_items (G_LIST_MODEL (self->other_types_model));

    if (result_count == 0)
    {
        gtk_stack_set_visible_child_name (self->type_dialog_stack, "empty_page");
        gtk_widget_set_sensitive (GTK_WIDGET (adw_dialog_get_default_widget (self->type_dialog)),
                                  FALSE);
    }
    else
    {
        gtk_stack_set_visible_child_name (self->type_dialog_stack, "results_page");
        gtk_widget_set_sensitive (GTK_WIDGET (adw_dialog_get_default_widget (self->type_dialog)),
                                  TRUE);
    }
}
...
show_other_types_dialog (NautilusSearchPopover *popover)
{
    ...
    for (GList *l = mime_infos; l != NULL; l = l->next)
    {
        ...
        g_autoptr (NautilusIconInfo) icon_info = nautilus_icon_info_lookup (icon, 32, 1);
    ...
    builder = gtk_builder_new_from_resource ("/org/gnome/nautilus/ui/nautilus-search-popover-types-dialog.ui");

                   ...
                   gpointer                       callback_data)
{
    ...
    for (l1 = files, l2 = new_names; l1 != NULL && l2 != NULL; l1 = l1->next, l2 = l2->next)
    {
        ...
        else
        {
            ...
            g_warning ("first: %s -> name: %s", path, new_name->str);
    ...
    g_warning ("colliding_files length: %d. fixed_filenames length: %d", colliding_count, g_list_length(fixed_filenames));
        ...
        while (l1)
        {
            ...
            if (strcmp(rename_couple->new_name, fixed_filename) == 0)
            {
                ...
                if (error == NULL)
                {
                    ...
                    path = g_file_get_path (location);
                    g_warning ("middle: %s -> name: %s", path, rename_couple->new_name);
                }
                else
                {
    ...
    while (colliding_files != NULL)
    {
        ...
        if (error == NULL)
        {
            ...
            path = g_file_get_path (location);
            g_warning ("colliding: %s -> name: %s", path, rename_couple->new_name);
        ...
        else if (g_error_matches (error, G_IO_ERROR, G_IO_ERROR_EXISTS))
        {
            ...
            while (random_named_file == NULL)
            {
                ...
                else
                {
                    ...
                    g_warning ("randomize: %s -> name: %s", path, name_with_suffix->str);

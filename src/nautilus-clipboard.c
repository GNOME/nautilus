...
struct _NautilusClipboardInfo
{
    gboolean cut;
    GList *files;
};

typedef struct _NautilusClipboardInfo NautilusClipboardInfo;
typedef struct _NautilusClipboardInfo ClipboardInfo;

static void
clipboard_info_free (ClipboardInfo *clipboard_info)
{
...
GType nautilus_clipboard_info_get_type (void) G_GNUC_CONST;
...
G_DEFINE_BOXED_TYPE (NautilusClipboardInfo, nautilus_clipboard_info,
                     clipboard_info_copy, clipboard_info_free)
                       ...
                       gconstpointer b)
{
    ...
    uri = nautilus_file_get_uri ((NautilusFile *) a);
...
clipboard_is_empty (GtkWidget *widget)
{
    ...
    return gdk_content_formats_contain_gtype (formats, NAUTILUS_TYPE_CLIPBOARD_INFO);
}
                                            ...
                                            const GList *item_uris)
{
    ...
    for (const GList *l = item_uris; l != NULL; l = l->next)
    {
                     ...
                     size_t                length)
{
    ...
    gdk_content_serializer_set_task_data (serializer, string, g_free);
}

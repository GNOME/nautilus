               ...
               GtkSnapshot        *snapshot)
{
        ...
        || canvas_item->details->is_highlighted_for_drop)
    {
        ...
        g_debug ("%s(): color: %f %f %f %f",
                 __func__, color.blue, color.red, color.green, color.alpha);

        graphene_matrix_init_from_float (&color_matrix, (float[16]) {
                color.blue, color.red, color.green, 0,
                color.blue, color.red, color.green, 0,
                color.blue, color.red, color.green, 0,
                0, 0, 0, color.alpha
        });
    ...
    gtk_snapshot_append_texture (snapshot, canvas_item->details->texture, &bounds);

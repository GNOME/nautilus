...
entry_would_have_inserted_characters (const GdkEvent *event)
{
    ...
    g_return_val_if_fail (gdk_event_get_keyval (event, &keyval), FALSE);
    ...
    /* TODO: gdk_event_get_string () and check if length > 0. */
                                  ...
                                  GdkEvent  *event)
{
    ...
    /* TODO: Calling the event vfunc is not enough, we need the entry

...
struct _NautilusTemplatesDialog
{
    ...
    NautilusDirectoryList *templates_directory_list;
...
G_DEFINE_FINAL_TYPE (NautilusTemplatesDialog, nautilus_templates_dialog, ADW_TYPE_DIALOG)
...
{
    g_autoptr (GFile) file = gtk_file_dialog_open_finish (chooser_dialog, result, NULL);
    ...
    if (file != NULL)
    {
        ...
        /* FIXME: Assumes the copy operation will succeed within 100ms */
        schedule_update (self);
    }
}

static void
nautilus_templates_dialog_import (NautilusTemplatesDialog *self)
{
...
{
    if (g_list_find (self->templates_directory_list, directory) != NULL)
    {
        return;
    }
    ...
    nautilus_directory_file_monitor_add (directory, &self->templates_directory_list,
                                         FALSE, attributes,
                                         (NautilusDirectoryCallback) templates_added_or_changed_callback, self);

    g_signal_connect_object (directory, "files-added",
                             G_CALLBACK (templates_added_or_changed_callback), self, G_CONNECT_DEFAULT);
    g_signal_connect_object (directory, "files-changed",
                             G_CALLBACK (templates_added_or_changed_callback), self, G_CONNECT_DEFAULT);

    self->templates_directory_list = g_list_append (self->templates_directory_list, directory);
}
...
remove_directory_from_templates_directory_list (NautilusTemplatesDialog *self,
                                                NautilusDirectory       *directory)
{
    self->templates_directory_list = g_list_remove (self->templates_directory_list, directory);
    g_signal_handlers_disconnect_by_func (directory,
                                          G_CALLBACK (templates_added_or_changed_callback),
                                          self);
    nautilus_directory_file_monitor_remove (directory, &self->templates_directory_list);
...
static GListModel *
templates_tree_list_model (gpointer item,
                           gpointer user_data)
{
    NautilusTemplatesDialog *self = NAUTILUS_TEMPLATES_DIALOG (user_data);
    NautilusFile *file = NAUTILUS_FILE (item);
    char *templates_directory_uri = nautilus_get_templates_directory_uri ();
    ...
    if (nautilus_file_is_directory (file))
    {
        ...
        {
            NautilusDirectory *dir = nautilus_directory_get_by_uri (uri);
...
update_templates_menu (NautilusTemplatesDialog *self)
{
    ...
    g_autolist (NautilusDirectory) sorted_copy = nautilus_directory_list_sort_by_uri (
    ...
    g_autoptr (NautilusDirectory) directory =
        nautilus_directory_get_by_uri (templates_directory_uri);
    GListStore *model = update_directory_in_templates_menu (self, directory);

    self->templates_list_model = gtk_tree_list_model_new (G_LIST_MODEL (model),
                                                          FALSE, FALSE,
                                                          templates_tree_list_model,
                                                          self, NULL);
...
static void
template_activated (GtkListBox    *self,
                    GtkListBoxRow *row,
                    gpointer       user_data)
{
    GtkTreeExpander *expander = GTK_TREE_EXPANDER (gtk_list_box_row_get_child (row));
    g_autoptr (NautilusFile) file = (NautilusFile *) gtk_tree_expander_get_item (expander);
...
templates_list_box_widgets (GObject  *item,
                            gpointer  user_data)
{
    NautilusFile *file = (NautilusFile *) gtk_tree_list_row_get_item (GTK_TREE_LIST_ROW (item));

    g_return_val_if_fail (NAUTILUS_IS_FILE (file), NULL);

    GtkWidget *expander = gtk_tree_expander_new ();
    GtkWidget *box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 12);
    GtkWidget *label = gtk_label_new (nautilus_file_get_display_name (file));
                ...
                gpointer  user_data)
{
    ...
    const gchar *file_name = nautilus_file_get_display_name (file);
                ...
                gpointer        user_data)
{
    ...
    {
        set_expanded_recursive (child_row, check);
        i += 1;
    }
...
static void
update_create_button_sensitivity (NautilusTemplatesDialog *self)
{
    gboolean passed = FALSE;
    g_autoptr (NautilusFile) file = NULL;

    g_object_get (self->validator, "passed", &passed, NULL);
    file = nautilus_templates_dialog_get_selected_file (self);

    gtk_widget_set_sensitive (self->create_button, passed && file != NULL);
}

static void
on_feedback_changed (NautilusTemplatesDialog *self)
{
...
static void
on_template_row_selected (GtkListBox    *list_box,
                          GtkListBoxRow *row,
                          gpointer       user_data)
{
    NautilusTemplatesDialog *self = NAUTILUS_TEMPLATES_DIALOG (user_data);
...
static void
template_create (NautilusTemplatesDialog *self)
{
    g_autoptr (NautilusFile) file = nautilus_templates_dialog_get_selected_file (self);
...
{
    GtkListBoxRow *row = gtk_list_box_get_selected_row (GTK_LIST_BOX (self->templates_list_box));
...
nautilus_templates_dialog_class_init (NautilusTemplatesDialogClass *klass)
{
    ...
    gtk_widget_class_bind_template_child (widget_class, NautilusTemplatesDialog, create_button);
    gtk_widget_class_bind_template_child (widget_class, NautilusTemplatesDialog, name_entry);
    gtk_widget_class_bind_template_child (widget_class, NautilusTemplatesDialog, validator);
    gtk_widget_class_bind_template_callback (widget_class, nautilus_templates_dialog_import);
    gtk_widget_class_bind_template_callback (widget_class, template_activated);
    gtk_widget_class_bind_template_callback (widget_class, template_create);
...
nautilus_templates_dialog_init (NautilusTemplatesDialog *self)
{
    ...
    g_signal_connect_object (self->templates_list_box, "row-selected",
                             G_CALLBACK (on_template_row_selected), self,
                             G_CONNECT_DEFAULT);
    g_signal_connect_object (self->validator, "notify::passed",
                             G_CALLBACK (update_create_button_sensitivity), self,
                             G_CONNECT_SWAPPED);
    update_create_button_sensitivity (self);

    self->filter = gtk_custom_filter_new ((GtkCustomFilterFunc) match_function,
                                          self->templates_search_entry, NULL);

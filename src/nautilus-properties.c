                       ...
                       GroupChange  *change)
{
    ...
    if (!change->cancelled)
    {
        ...
        nautilus_ui_timed_wait_stop ((TimedWaitCancelCallback) cancel_group_change_callback, change);
...
schedule_group_change_timeout (GroupChange *change)
{
    ...
    nautilus_ui_timed_wait_start ((TimedWaitCancelCallback) cancel_group_change_callback,
                                  change,
                       ...
                       OwnerChange  *change)
{
    ...
    {
        /* Report the error if it's an error. */
        nautilus_ui_timed_wait_stop ((TimedWaitCancelCallback) cancel_owner_change_callback, change);
...
schedule_owner_change_timeout (OwnerChange *change)
{
    ...
    nautilus_ui_timed_wait_start ((TimedWaitCancelCallback) cancel_owner_change_callback,
                                  change,
...
static void
setup_properties_widget (NautilusPropertiesWidget *self)
{
    for (NautilusFileList *l = self->files; l != NULL; l = l->next)
    {
        NautilusFile *file = l->data;
        NautilusAttributes attributes = NAUTILUS_ATTRIBUTE_THUMBNAIL_BUFFER;
    ...
    /* Create the pages. */
    setup_basic_page (self);

    if (should_show_permissions (self))
    {
        setup_permissions_page (self);
        gtk_widget_set_visible (self->permissions_navigation_row, TRUE);
    }
    ...
    properties_widget_update (self, NULL);
}

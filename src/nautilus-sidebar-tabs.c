...
nautilus_index_tabs_initialize (NautilusIndexTabs *index_tabs)
{
	...
	gdk_color_parse ("rgb:d6/d6/d6", &index_tabs->details->hilite_color);
	gdk_colormap_alloc_color (gtk_widget_get_colormap (GTK_WIDGET (index_tabs)), 
				  &index_tabs->details->hilite_color, FALSE, TRUE);
...
int nautilus_index_tabs_hit_test(NautilusIndexTabs *index_tabs, double x, double y)
{
...
{  
	...
	gdk_gc_set_foreground (gc, &index_tabs->details->hilite_color);  
...
void
nautilus_index_tabs_set_visible(NautilusIndexTabs* index_tabs, const char *name, gboolean is_visible)
{
	/* first, look up the item */
	tabItem *this_item;
	GList *item = find_tab (index_tabs, name);

                           ...
                           NautilusPropertiesWindow *window)
{
    ...
    if (gtk_toggle_button_get_active (button)
        && !gtk_check_button_get_inconsistent (GTK_CHECK_BUTTON (button)))
    {
    ...
    else if (gtk_check_button_get_inconsistent (GTK_CHECK_BUTTON (button))
                          ...
                          GtkToggleButton          *button)
{
    ...
    gtk_check_button_set_inconsistent (GTK_CHECK_BUTTON (button),
                                        (!all_unset && !all_set) ||
                                        (!is_folder && no_match));
...
create_permissions_page (NautilusPropertiesWindow *window)
{
    ...
    if (all_can_get_permissions (file_list) && all_can_get_permissions (window->target_files))
    {
        ...
        gtk_widget_set_hexpand (GTK_WIDGET (page_grid), TRUE);
...
create_properties_window (StartupData *startup_data)
{
    ...
    gtk_widget_set_hexpand (GTK_WIDGET (window->notebook), TRUE);
                                    ...
                                    const gchar *startup_id)
{
    ...
    if (existing_window != NULL)
    {
        if (parent_widget)
        {
            gtk_window_set_display (existing_window,

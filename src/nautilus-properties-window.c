                ...
                const gchar              *name)
{
    ...
    stack_child_label = gtk_stack_get_child_by_name (GTK_STACK (window->name_stack), "name_value_label");
    stack_child_entry = gtk_stack_get_child_by_name (GTK_STACK (window->name_stack), "name_value_entry");
    ...
    if (use_label)
    {
        ...
        gtk_stack_set_visible_child (GTK_STACK (window->name_stack), stack_child_label);
    }
    else
    {
        gtk_stack_set_visible_child (GTK_STACK (window->name_stack), stack_child_entry);
    }
...
{
    NautilusPropertiesWindow *window;
    GtkWidget *name_field_entry;

    window = NAUTILUS_PROPERTIES_WINDOW (callback_data);
    name_field_entry = gtk_stack_get_child_by_name (GTK_STACK (window->name_stack), "name_value_entry");
...
create_basic_page (NautilusPropertiesWindow *window)
{
    ...
    if (GTK_IS_ENTRY (gtk_stack_get_visible_child (GTK_STACK (window->name_stack))))
    {
        gtk_widget_grab_focus (GTK_WIDGET (gtk_stack_get_visible_child (GTK_STACK (window->name_stack))));
    }
    ...
    if (should_show_trashed_on (window))
    {
        ...
        g_object_set_data_full (G_OBJECT (window->trashed_on_field), "inconsistent_string",
                                g_strdup (INCONSISTENT_STATE_STRING), g_free);

        g_object_set_data (G_OBJECT (window->trashed_on_field), "show_original", GINT_TO_POINTER (FALSE));

        window->value_fields = g_list_prepend (window->value_fields,
                                               window->trashed_on_field);
    }

...
struct _NautilusPropertiesWindow
{
    ...
    GtkWidget *owner_access_combo_box;
    GtkWidget *owner_folder_access;
    GtkWidget *owner_folder_access_combo_box;
    GtkWidget *owner_file_access;
    GtkWidget *owner_file_access_combo_box;

    GtkWidget *group_value_stack;
    GtkWidget *group_access;
    GtkWidget *group_access_combo_box;
    GtkWidget *group_folder_access;
    GtkWidget *group_folder_access_combo_box;
    GtkWidget *group_file_access;
    GtkWidget *group_file_access_combo_box;
...
static void
setup_owner_combo_box (NautilusPropertiesWindow *window,
                       NautilusFile             *file)
{
    GtkWidget *combo_box;
    GtkTreeModel *model;
    ...
    combo_box = gtk_stack_get_child_by_name (GTK_STACK (window->owner_value_stack), "combo_box");
    gtk_stack_set_visible_child (GTK_STACK (window->owner_value_stack), combo_box);
...
static void
setup_permissions_combo_box (NautilusPropertiesWindow *window,
                             ...
                             gboolean                  is_folder)
{
    ...
    gtk_combo_box_set_id_column (GTK_COMBO_BOX (combo_box), COLUMN_ID);

    g_object_set_data (G_OBJECT (combo_box), "is-folder", GINT_TO_POINTER (is_folder));
    g_object_set_data (G_OBJECT (combo_box), "permission-type", GINT_TO_POINTER (type));
    ...
    window->permission_combos = g_list_prepend (window->permission_combos,
                                                combo_box);
    g_signal_connect (combo_box, "changed", G_CALLBACK (permission_combo_changed), window);
                           ...
                           GtkGrid                  *page_grid)
{
    ...
    if (!is_multi_file_window (window) && nautilus_file_can_set_owner (get_target_file (window)))
    {
        ...
        setup_owner_combo_box (window, get_target_file (window));
    }
    else
    {
        /* Static text in this case. */
        value = gtk_stack_get_child_by_name (GTK_STACK (window->owner_value_stack), "owner_static_label");
        gtk_stack_set_visible_child (GTK_STACK (window->owner_value_stack), value);

        /* Stash a copy of the file attribute name in this field for the callback's sake. */
        g_object_set_data_full (G_OBJECT (value), "file_attribute",
                                g_strdup ("owner"), g_free);

        g_object_set_data_full (G_OBJECT (value), "inconsistent_string",
                                g_strdup (INCONSISTENT_STATE_STRING), g_free);

        g_object_set_data (G_OBJECT (value), "show_original", GINT_TO_POINTER (FALSE));
    ...
    {
        gtk_widget_show (window->owner_folder_access);
        gtk_widget_show (window->owner_folder_access_combo_box);
        setup_permissions_combo_box (window, GTK_COMBO_BOX (window->owner_folder_access_combo_box),
                                     PERMISSION_USER, TRUE);
        gtk_widget_show (window->owner_file_access);
        gtk_widget_show (window->owner_file_access_combo_box);
        setup_permissions_combo_box (window, GTK_COMBO_BOX (window->owner_file_access_combo_box),
                                     PERMISSION_USER, FALSE);
    }
    else
    {
        gtk_widget_show (window->owner_access);
        gtk_widget_show (window->owner_access_combo_box);
        setup_permissions_combo_box (window, GTK_COMBO_BOX (window->owner_access_combo_box),
    ...
    if (!is_multi_file_window (window) && nautilus_file_can_set_group (get_target_file (window)))
    {
        ...
        setup_group_combo_box (group_combo_box, get_target_file (window), FALSE);
    }
    else
    {
    ...
    if (has_directory && has_file)
    {
        gtk_widget_show (window->group_folder_access);
        gtk_widget_show (window->group_folder_access_combo_box);
        setup_permissions_combo_box (window, GTK_COMBO_BOX (window->group_folder_access_combo_box),
                                     PERMISSION_GROUP, TRUE);
        gtk_widget_show (window->group_file_access);
        gtk_widget_show (window->group_file_access_combo_box);
        setup_permissions_combo_box (window, GTK_COMBO_BOX (window->group_file_access_combo_box),
                                     PERMISSION_GROUP, FALSE);
    }
    else
    {
        gtk_widget_show (window->group_access);
        gtk_widget_show (window->group_access_combo_box);
        setup_permissions_combo_box (window, GTK_COMBO_BOX (window->group_access_combo_box),
...
nautilus_properties_window_class_init (NautilusPropertiesWindowClass *klass)
{
    ...
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, owner_access);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, owner_folder_access);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, owner_file_access);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, owner_access_combo_box);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, owner_folder_access_combo_box);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, owner_file_access_combo_box);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, group_value_stack);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, group_access);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, group_folder_access);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, group_file_access);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, group_access_combo_box);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, group_folder_access_combo_box);
    gtk_widget_class_bind_template_child (widget_class, NautilusPropertiesWindow, group_file_access_combo_box);
}

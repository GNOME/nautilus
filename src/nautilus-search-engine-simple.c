...
static void
call_idle (SearchThreadData *thread_data,
           GList            *hits,
           gboolean          done)
{
    gboolean is_empty;
    ...
    is_empty = g_queue_is_empty (thread_data->idle_queue);
    if (hits)
    {
...
send_batch (SearchThreadData *thread_data)
{
    ...
    if (thread_data->hits)
    {
        call_idle (thread_data, thread_data->hits, FALSE);
    }

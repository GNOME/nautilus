...
/* We initializing these only sparingly to increase performance. */
             ...
             gboolean   detailed_date)
{
    ...
    if (U_FAILURE (status))
    {
        g_warning ("ICU failed to create a formatted date: %s", u_errorName (status));
    }

...
#include "nautilusgtkfilesystem.h"
...
struct _NautilusGtkPlacesSidebar {
  ...
  NautilusGtkTrashMonitor   *trash_monitor;
  ...
  if (sidebar->trash_row)
    {
      ...
      icon = _nautilus_gtk_trash_monitor_get_icon (sidebar->trash_monitor);
...
update_places (NautilusGtkPlacesSidebar *sidebar)
{
  ...
  if (!sidebar->local_only && sidebar->show_trash)
    {
      start_icon = _nautilus_gtk_trash_monitor_get_icon (sidebar->trash_monitor);
                              ...
                              GdkDrag          *drag)
{
  ...
  for (l = rows; l != NULL; l = l->next)
    {
      sensitive = !dragging || check_valid_drop_target (sidebar, NAUTILUS_GTK_SIDEBAR_ROW (l->data), drag);
                      ...
                      gpointer   user_data)
{
  ...
  if (!check_valid_drop_target (sidebar, NAUTILUS_GTK_SIDEBAR_ROW (row), drag))
    goto out;
 ...
 out:
  start_drop_feedback (sidebar, NAUTILUS_GTK_SIDEBAR_ROW (row), drag);
                        ...
                        gpointer          user_data)
{
  ...
  if (target == g_intern_static_string ("DND_NAUTILUS_GTK_SIDEBAR_ROW"))
    {
                             ...
                             gpointer          user_data)
{
  ...
  if (!sidebar->drag_data_received)
    {
      if (gtk_selection_data_targets_include_uri (selection_data))
        ...
        {
          sidebar->drag_list = NULL;
          if (gtk_selection_data_get_target (selection_data) == g_intern_static_string ("DND_NAUTILUS_GTK_SIDEBAR_ROW"))
            sidebar->drag_data_info = DND_NAUTILUS_GTK_SIDEBAR_ROW;
        }
  ...
  if (!check_valid_drop_target (sidebar, NAUTILUS_GTK_SIDEBAR_ROW (target_row), gdk_drop_get_drag (drop)))
    goto out;
               ...
               NautilusGtkPlacesSidebar *sidebar)
{
  ...
  if (NAUTILUS_GTK_IS_SIDEBAR_ROW (row))
    popup_menu_cb (NAUTILUS_GTK_SIDEBAR_ROW (row));
}
...
nautilus_gtk_places_sidebar_init (NautilusGtkPlacesSidebar *sidebar)
{
  ...
  sidebar->trash_monitor = _nautilus_gtk_trash_monitor_get ();
...
nautilus_gtk_places_sidebar_set_open_flags (NautilusGtkPlacesSidebar   *sidebar,
                                   GtkPlacesOpenFlags  flags)
{
  g_return_if_fail (NAUTILUS_GTK_IS_PLACES_SIDEBAR (sidebar));
...
{
  g_return_val_if_fail (NAUTILUS_GTK_IS_PLACES_SIDEBAR (sidebar), 0);
...
{
  g_return_val_if_fail (NAUTILUS_GTK_IS_PLACES_SIDEBAR (sidebar), FALSE);
...
{
  g_return_val_if_fail (NAUTILUS_GTK_IS_PLACES_SIDEBAR (sidebar), TRUE);
...
{
  g_return_val_if_fail (NAUTILUS_GTK_IS_PLACES_SIDEBAR (sidebar), NULL);

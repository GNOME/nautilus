...
is_wanted_by_monitor (NautilusFile *file,
                      ...
                      NautilusFile *monitor_file) {
    NautilusDirectory *directory;
    GList *node;

    directory = file->details->directory;
    node = g_hash_table_lookup (directory->details->monitor_table,
                                monitor_file);
          ...
          RequestType   request_type_wanted)
{
    ...
    {
        if (is_wanted_by_monitor (file, request_type_wanted, file))
        {
            return TRUE;
        }
        /* monitor->file == NULL means monitor all files. */
        if (is_wanted_by_monitor (file, request_type_wanted, NULL))
        {
            return TRUE;
        }
    }

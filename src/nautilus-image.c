...
#define LOADING_COUNT_LIMIT 10
#define CACHE_COUNT_LIMIT 1000
#define MAX_IMAGE_SIZE (8 * 1024 * 1024) /* 8 MB */
...
thumbnail_cache_get (GFile *file)
{
    ...
    if (item != NULL)
    {
        nautilus_hash_queue_move_existing_to_head (thumbnail_cache, item->file);
    }
                     ...
                     guint64     mtime)
{
    ...
    if (old_item != NULL)
    {
        ...
        nautilus_hash_queue_move_existing_to_head (thumbnail_cache, file);
                               ...
                               const gchar   *icon_name)
{
    ...
    g_autoptr (GIcon) error_icon = g_themed_icon_new (icon_name);
...
{
    static GdkPaintable *loading_paintable;

    if (G_LIKELY (loading_paintable != NULL))
    {
        return loading_paintable;
    }

    loading_paintable = get_paintable_for_themed_icon (self, "image-loading");
...
{
    static GdkPaintable *error_paintable;

    if (G_LIKELY (error_paintable != NULL))
    {
        return error_paintable;
    }

    error_paintable = get_paintable_for_themed_icon (self, "image-missing");
                          ...
                          gpointer      user_data)
{
    ...
    goffset file_size = g_file_info_has_attribute (info, G_FILE_ATTRIBUTE_STANDARD_SIZE)
        ...
        nautilus_thumbnail_is_mimetype_limited_by_size (self->source_content_type))
    {
        g_set_error (&self->error,
                    G_IO_ERROR,
                    G_IO_ERROR_FAILED,
                    "Image file is too large: %" G_GOFFSET_FORMAT " bytes (max: %d bytes)",
               ...
               GtkSnapshot *snapshot)
{
    ...
    if (self->texture != NULL)
    {
        ...
        int widget_scale = gtk_widget_get_scale_factor (widget);
        GskRoundedRect rounded_rect;

        width = width / widget_scale;
        height = height / widget_scale;
        ...
        gtk_snapshot_pop (snapshot);
             ...
             self->error == NULL)
    {
        GdkPaintable *paintable = get_loading_paintable (self);
    ...
    else
    {
        ...
        {
            paintable = g_object_ref (self->fallback_paintable);
        }

        if (paintable == NULL)
        {
            paintable = g_object_ref (get_error_paintable (self));
        }
	      ...
	      int            *natural_baseline)
{
    ...
    if (self->texture !=  NULL)
    {
        ...
        if (MAX (width, height) != self->size)
        {
            float scale_factor2 = MAX (width, height) / self->size;

            width = width / scale_factor2;
            height = height / scale_factor2;
        }

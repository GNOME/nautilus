                  ...
                  GtkTreePath              *path)
{
    ...
    target = gtk_selection_data_get_target (dest->details->drag_data);
    ...
    if (target == g_intern_static_string (NAUTILUS_ICON_DND_GNOME_ICON_LIST_TYPE))
    {
        actions = nautilus_get_drop_actions_for_icons (drop,
                                                       drop_target,
                                                       dest->details->drag_list,
                                                       0);
    }
    else if (gtk_selection_data_targets_include_text (dest->details->drag_data))
    {
        actions = GDK_ACTION_COPY;
    }
    else if (gtk_selection_data_targets_include_uri (dest->details->drag_data))
    {
              ...
              int                       y)
{
    ...
    if (gtk_selection_data_targets_include_uri (dest->details->drag_data))
    {
        actions = GDK_ACTION_COPY;
    }

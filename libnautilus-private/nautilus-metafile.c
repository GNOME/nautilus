...
construct_private_metafile_uri (const char *uri)
{
	...
	metafiles_directory = g_strconcat (user_directory, "/", METAFILES_DIRECTORY_NAME, NULL);
	...
	alternate_path = g_strconcat (metafiles_directory, "/", file_name, NULL);
				     ...
				     const char *directory_uri)
{
	...
	g_free (metafile->details->public_uri);
	metafile->details->public_uri = g_strconcat
		       ...
		       const char *file_name)
{
	...
	return g_strconcat (metafile->details->directory_uri, "/", file_name, NULL);
}

			    ...
			    NautilusLinkType type)
{
	...
	retval = nautilus_link_impl_desktop_local_create (directory_path,
							  name, image,
							  target_uri, point,
							  type);
			      ...
			      NautilusLinkType type)
{
	...
	if (strcmp (mime_type, "application/x-nautilus-link") == 0) {
		...
		retval = nautilus_link_impl_desktop_local_set_type (path, type);
	}
...
nautilus_link_local_get_additional_text (const char *path)
{
	...
	if (strcmp (mime_type, "application/x-nautilus-link") == 0) {
		...
		retval = nautilus_link_impl_desktop_local_get_additional_text (path);
	}
...
nautilus_link_local_get_link_uri (const char *path)
{
	...
	if (strcmp (mime_type, "application/x-nautilus-link") == 0) {
		...
		retval = nautilus_link_impl_desktop_local_get_link_uri (path);
	}
...
nautilus_link_local_get_link_type (const char *path)
{
	...
	if (strcmp (mime_type, "application/x-nautilus-link") == 0) {
		...
		retval = nautilus_link_impl_desktop_local_get_link_type (path);
	}
...
nautilus_link_local_is_volume_link (const char *path)
{
	...
	if (strcmp (mime_type, "application/x-nautilus-link") == 0) {
		...
		retval = nautilus_link_impl_desktop_local_is_volume_link (path);
	}
...
nautilus_link_local_is_home_link (const char *path)
{
	...
	if (strcmp (mime_type, "application/x-nautilus-link") == 0) {
		...
		retval = nautilus_link_impl_desktop_local_is_home_link (path);
	}
...
nautilus_link_local_is_trash_link (const char *path)
{
	...
	if (strcmp (mime_type, "application/x-nautilus-link") == 0) {
		...
		retval = nautilus_link_impl_desktop_local_is_trash_link (path);
	}
...
{
	nautilus_link_impl_desktop_local_create_from_gnome_entry (entry, dest_path, position);
}

...
GtkWidget*
ammonite_login_dialog_new (const char	*dialog_title,
			      const char	*message,
			      const char	*username,
			      const char	*password,
			      ...
			      gboolean		has_remember_box)
{
	...
	/* Make hbox to hold table and pixmap */

	/* FIXME bugzilla.eazel.com 2769: this needs to get the image in a more guarenteed way */
	pixmap = gnome_pixmap_new_from_file ( DATADIR "/pixmaps/nautilus/nautilus-logo.png" );
	...
	gtk_box_pack_end (GTK_BOX(hbox), password_dialog->details->table, TRUE, TRUE, 0);
	...
	gtk_box_pack_start (GTK_BOX (GNOME_DIALOG (password_dialog)->vbox),
			    hbox/*password_dialog->details->table*/,
			    TRUE,	/* expand */
			    TRUE,	/* fill */
			    0);		/* padding */
